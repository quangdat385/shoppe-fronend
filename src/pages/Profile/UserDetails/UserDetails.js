...
function UserDetails({ user, setQuery }) {
  ...
  const handleUpdateUser = async () => {
    try {
      const result = await updateUser({ id: user._id, ...updateFields }).unwrap();
  ...
  const updateEmail = async () => {
    try {
      const result = await updateEmailFn({ id: user._id, email: email }).unwrap();
  ...
  const verifyOtp = async () => {
    ...
    try {
      const result = await updatePhoneNumber({ id: user._id, phone_number: phoneNumber }).unwrap();
      signOut(auth)
      setResult(result);
      setErrMsg("")
      setSection("profile")
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (result.status === 200) {
                      setQuery(pre => {
                        return { ...pre, update: !pre.update, id }
                      })
                    }
