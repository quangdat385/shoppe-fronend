...
function UpdateAddress({ user, hidden, setHidden, type, addressId, setQuery }) {
  ...
  useEffect(() => {
    setAddress(pre => {
      return { ...pre, fullName, phoneNumber, typeAddress, deFault, details }
    })
  }, [fullName, phoneNumber, deFault, typeAddress, details])
  ...
  const handleUpdateAddress = async () => {
    ...
    try {
      await updateAddress({ id: user._id, addressId, address: address }).unwrap();
      setErrMsg("");
      setQuery(pre => {
