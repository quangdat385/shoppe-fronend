...
function Address({ user, setQuery }) {
  ...
  const handleUpdateAddress = async () => {
    ...
    try {
      ...
      await addAddress({ id: user._id, address: address }).unwrap();
      setErrMsg("");
      setQuery(pre => {
        return { ...pre, update: !pre.update, id: user._id }
      })
  ...
  return (<Container className={cx("address-wrapper")}>
    <Container>
      <div className={cx('header')}>
        ...
        <UpdateAddress setQuery={setQuery} addressId={addressId} user={user} hidden={visible} setHidden={setVisible} type="Nhà Riêng" />
        <div
                        ...
                        <div className={cx('btn-wrapper')}>
                          ...
                          <div
                            onClick={async () => {
                              ...
                              try {
                                console.log("User", user._id);
                                await deleteAddress({ id: user._id, addressId: item._id }).unwrap();
                                setQuery(pre => {
                                  return { ...pre, update: !pre.update, id: user._id }
                                })
                                navigate("/user/profile");
